...
public partial class TallyViewModel : ViewModelBase
{
    ...
    public ICommand ShowTallyMenuCommand => _showTallyMenuCommand ??= new RelayCommand<TallyPopulation>((tp) => ShowTallyMenu(tp).FireAndForget());

    public ICommand TallyCommand => _tallyCommand ??= new RelayCommand<TallyPopulation>((tp) => TallyAsync(tp).FireAndForget("TallyAsync"));

    public ICommand EditTreeCommand => _editTreeCommand ??= new RelayCommand<string>((treeID) => EditTree(treeID).FireAndForget());
