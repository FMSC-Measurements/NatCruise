...
public class PlotEditViewModel : ViewModelBase
{
    ...
    private ICommand? _toggleInCruiseCommand;
    ...
    public ICommand UpdatePlotNumberCommand => _updatePlotNumberCommand ??= new RelayCommand<string>(UpdatePlotNumber);

    public ICommand ShowLimitingDistanceCommand => _showLimitingDistanceCommand ??= new RelayCommand<Plot_Stratum>(x => ShowLimitingDistanceCalculatorAsync(x).FireAndForget());

    public ICommand ShowPlotTallyCommand => _showPlotTallyCommand ??= new RelayCommand(() => NavigationService.ShowPlotTally(Plot?.PlotID).FireAndForget());

    public ICommand ToggleInCruiseCommand => _toggleInCruiseCommand ??= new RelayCommand<Plot_Stratum>((x) => ToggleInCruiseAsync(x).FireAndForget());
    ...
    private void StratumPlot_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
